...
#include "kgu_islab_erasure_Decoder.h"
...
int i;
	jsize arrSize = (*env)->GetArrayLength(env,args);
	const char *p[arrSize];
	jstring str[arrSize];
	for(i=0;i<arrSize;++i){
		str[i] = (jstring)(*env)->GetObjectArrayElement(env,args,i);
		p[i] = (*env)->GetStringUTFChars(env,str[i],0);	
	}
	decoder_main(arrSize,p);
	for(i=0;i<arrSize;++i)
		(*env)->ReleaseStringUTFChars(env,str[i],p[i]);
	return (*env)->NewStringUTF(env,"decoding_return");
